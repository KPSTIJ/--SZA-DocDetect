...
- **Detection methods** — `detection_methods` в JobSummary, тэги Text/Fusion/VLM/Manual на карточках, фильтр по методу
- **Постраничный вывод** — 52 досье/страница, пагинация сверху и снизу, сортировка по дате
- **Навигация по досье** — стрелки и клавиши `←`/`→` для переключения между досье
- **Массовое подтверждение** — batch confirm-correct + групповой выбор чекбоксами на карточках
- **Кеш страниц** — prefetch + cache 10 последних досье, инвалидация при изменениях
- **Локальное + SMB сохранение** — файлы сохраняются и локально, и на SMB
- **SMB delete** — рекурсивное удаление папок через `FILE_DELETE_ON_CLOSE`
...
| **Одинаковые типы подряд не разделялись по is_title_page** | `_revalidate_lengths` и `_assign_document_boundaries` разбивают сегменты по `is_title_page` |
| **Тип распознавания не отображался** | Добавлено поле `detection_methods` в JobSummary, тэги Text/Fusion/VLM/Manual на карточках |
| **Фильтр по методу распознавания** | Select с Text/Fusion/VLM/Manual в панели фильтров |
| **Не работал confirm при SMB** | `save_file` переведён на `FILE_OVERWRITE_IF`, `assemble_after_review` обёрнут в try/except |
| **SMB delete не работал** | `_delete_dir_recursive` использует `FILE_DELETE_ON_CLOSE` в `create_options` |
| **Confirm не переводил в done** | `batch/confirm-correct` теперь вызывает `assemble_after_review` + `job.status=done` |
| **Delete batch падал с 500** | Добавлен `selectinload(ProcessingJob.output_documents)` для eager loading |
| **Папки не удалялись при delete** | `_cleanup_output_dirs` теперь использует `shutil.rmtree` + вычисляемый путь `final_output_dir/stem` |
| **MissingGreenlet при delete** | Добавлен `db.refresh(job, ['output_documents'])` в single delete |
...
- **Нумерация документов** — `№{index}` перед названием для всех документов, разделитель `/` между номером и типом
- **Кеш страниц** — pagesCache на 10 записей с автоочисткой, префетч соседних досье, инвалидация при изменениях
- **Постраничный вывод** — 52 досье на страницу, пагинация сверху и снизу
- **Сортировка** — кнопка «Новые»/«Завершённые» по `created_at`/`finished_at`
- **Навигация по досье** — стрелки `<` `>` в шапке модалки + клавиши `←` `→`
- **Групповой чекбокс** — выбор всех страниц типа через Checkbox в заголовке группы
- **Массовое подтверждение корректности** — чекбоксы на карточках, плавающая панель с кнопкой, вызов `batch/confirm-correct`
- **Подтвердить разрезание** — кнопка в правом углу шапки модалки, disabled пока есть ошибки
- **Методы распознавания** — цветные тэги Text/Fusion/VLM/Manual на карточках и фильтр
- **Клавиатурная навигация** — `←`/`→` переключение досье, `↑`/`↓` навигация по превью страниц (без конфликта)
- **userSelect: none** — запрет выделения текста при клике на типах документов
- **Локальное сохранение** — файлы сохраняются и локально, и на SMB (раньше только SMB)
